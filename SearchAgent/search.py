...
def graph_search(problem, g, h, verbose=False, debug=True):
    """
    ...
    """
    #Three things before entering the loop: Initialize the first node, create an explored set, and creating a frontier set with the first node
    node = SearchNode(problem,problem.getStartState(),None,None,g,h)

    #Begin by filling the initial frontier state with the start state
    frontier = util.PriorityQueue()
    frontier.push(node,node.get_f())

    #Initialize the explored set
    explored = Explored
    explored.__init__(explored)


    while True:
        #if the frontier becomes empty without finding the goal, then the search has failed. Return none
        if frontier.isEmpty(): return None
        successorNode = SearchNode

        #SEARCH:
        #1. Set the current node to be the top of the frontier set (this will be the node with the lowest cost)
        node = frontier.pop()
        explored.add(explored,node.get_state())
        if problem.isGoalState(node.get_state()):
            return node.get_path()
        else:

            #Push the current node's successors onto a priority queue
            successors = problem.getSuccessors(node.get_state())

            #Union of frontier and explored
            for successor in successors:
                if not (explored.exists(explored,successor[0])):
                    frontier.push(SearchNode(problem,successor[0],node,successor[1],g,h),node.get_f())
...
class DepthFirstSearch:
    ...
    def g(cls, node: SearchNode):
        """
        For a depth first serach, our g is the depth of our search node
        """
        return 0
        raise NotImplementedError()
    ...
    def h(cls, node: SearchNode, problem):
        """
        For a depth first search, our heuristic is a constant, 0 in our case
        """
...
class AStarSearch:
    ...
    def h(cls, node: SearchNode, problem):
        ...
        dist = np.sqrt(((goalState[0]-nodeState[0])^2) + (goalState[1]-nodeState[1])^2)
        return dist
